...
EOF
    for entry in "${log_roots[@]}"; do
      ...
      escaped_root="${escaped_root//\"/\\\"}"
      cat >> "$output_dir/alloy.alloy" <<EOF
    { __path__ = "$escaped_root/*/diagnostics/investigation/*.jsonl", cluster = "$cluster_name", node = "$node_name", workload = "$workload", signal = "xlayer_diagnosis" },
    { __path__ = "$escaped_root/*/telemetry/diagnostics/investigation/*.jsonl", cluster = "$cluster_name", node = "$node_name", workload = "$workload", signal = "xlayer_diagnosis" },
EOF
    done
    cat >> "$output_dir/alloy.alloy" <<EOF
  ]
  forward_to = [loki.process.diagnosis.receiver]
  file_match {
    enabled = true
    ignore_older_than = "${ALLOY_IGNORE_OLDER_THAN:-24h}"
    sync_period = "2s"
...
loki.source.file "workload_events" {
  targets = [
EOF
    for entry in "${log_roots[@]}"; do
      workload="${entry%%=*}"
      ...
      escaped_root="${escaped_root//\"/\\\"}"
      cat >> "$output_dir/alloy.alloy" <<EOF
    { __path__ = "$escaped_root/*/telemetry-events/*.jsonl", __path_exclude__ = "$escaped_root/*/telemetry-events/verl-steps*.jsonl", cluster = "$cluster_name", node = "$node_name", workload = "$workload", signal = "xlayer_event" },
    { __path__ = "$escaped_root/*/telemetry/telemetry-events/*.jsonl", __path_exclude__ = "$escaped_root/*/telemetry/telemetry-events/verl-steps*.jsonl", cluster = "$cluster_name", node = "$node_name", workload = "$workload", signal = "xlayer_event" },
EOF
    done
    cat >> "$output_dir/alloy.alloy" <<EOF
  ]
  forward_to = [loki.process.workload_events.receiver]
  file_match {
    enabled = true
    ignore_older_than = "${ALLOY_IGNORE_OLDER_THAN:-24h}"
    sync_period = "2s"
...
loki.process "workload_events" {
  forward_to = [loki.write.monitoring_host.receiver]
  stage.json {
    expressions = { event_time_unix_nano = "event_time_unix_nano" }
  }
  stage.timestamp {
    source = "event_time_unix_nano"
    format = "UnixNs"
  }
  stage.label_drop {
    values = ["filename"]
