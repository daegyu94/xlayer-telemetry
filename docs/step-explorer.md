...
Step Explorer는 완료된 VERL step을 골라 stage 소요 시간, 같은 시간대의 자원 표본, Loki log를 한 화면에서 비교하는 로컬 읽기 전용 UI입니다.
...
Agent RL 대시보드의 `Step Explorer (local)` 링크도 같은 주소를 엽니다.
...
GPU·host·network·disk는 선택한 node 전체 또는 그 node의 process 합계이며, vLLM은 공유 engine 신호입니다.
